...
class HermesAdapter:
    ...
    Modes:
    - stub (default): deterministic local response for tests and offline dev.
    ...
    def __init__(self, mode: str | None = None, timeout_seconds: int | None = None) -> None:
        self.mode = (mode or os.getenv("CLAWDESK_HERMES_MODE") or "stub").strip().lower()
        raw_timeout = os.getenv("CLAWDESK_HERMES_TIMEOUT_SECONDS")
        self.timeout_seconds = timeout_seconds or int(raw_timeout or "60")
    ...
    def _invoke_cli(self, request: HermesRequest) -> HermesResponse:
        command = ["hermes", "chat", "-q", request.prompt, "--profile", request.profile]
        if request.model:
        ...
        return HermesResponse(text=stdout, status="succeeded")
